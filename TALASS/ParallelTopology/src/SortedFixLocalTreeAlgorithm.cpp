...
void SortedFixLocalTreeAlgorithm::constructAMtFromTokens(MergeTree* AMt, 
                                                   SortedInputStream* inputs)
{ 
  ...
  while (it.type() != EMPTY) {  
    ...
    switch(it.type()) {
      ...
      case Token::EDGE: {
        ...
        //std::cout << "Edge\n";
        TreeNode* high = AMt->findElement(e[0]);
        TreeNode* low;
        if (last_seen_node->id() == e[1])
          low = last_seen_node;
        else
          low = AMt->findElement(e[1]);

        AMt->addEdge(high, low); // (high, low)
        break;
        }
...
bool SortedFixLocalTreeAlgorithm::isNonLocalLeafNode(TreeNode* node) {
...
bool SortedFixLocalTreeAlgorithm::isNonLocalMinima(MergeTree *LTree,
                                             TreeNode* node) {
...
int SortedFixLocalTreeAlgorithm::apply(MergeTree& tree,
                                       SortedInputStream* inputs,
                                       TopoOutputStream* outputs)
{
 ...
 // }
  ...
  //if (mLTree->id() == 28) {
  if (mTreeNum == 0) {
    if (!mSortedVertexValArray.empty()) {
      //std::cout << "minina :: " << mSortedVertexValArray.back().idx
      //          << " Vaal:: " << mSortedVertexValArray.back().val << std::endl;
      mLTree->minimaVertex.idx = mSortedVertexValArray.back().idx;
      mLTree->minimaVertex.val = mSortedVertexValArray.back().val;
    }
  }
...
/*
  ...
  std::cout          << " Local Nodes :: " << mSortedVertexValArray.size()//LocalNodes
...
bool SortedFixLocalTreeAlgorithm::isMaxOrSaddle(TreeNode* node){
...
void SortedFixLocalTreeAlgorithm::checkParents(TreeNode* NodeLT, TreeNode* NodeAMt, 
                                         MergeTree* AMt){
...
bool SortedFixLocalTreeAlgorithm::checkDescendant(TreeNode* newParentFromAMt, 
...
TreeNode* SortedFixLocalTreeAlgorithm::addEdge(TreeNode* head,TreeNode* tail)
{
...
TreeNode* SortedFixLocalTreeAlgorithm::findIntegrationVertex(TreeNode* high, 
                                                       TreeNode* low)
{
...
TreeNode* SortedFixLocalTreeAlgorithm::mergeBranches(TreeNode* left,
    TreeNode* right) {
  ...
  while ((left != right) && (getChild(left) != NULL)) {
    ...
    left = SortedFixLocalTreeAlgorithm::findIntegrationVertex(left, right);
  }
...
int SortedFixLocalTreeAlgorithm::finalizeVertex(TreeNode* v) {
...
bool SortedFixLocalTreeAlgorithm::isRegular(TreeNode* v) const {
  if (getChild(v) == NULL)
    return false;
